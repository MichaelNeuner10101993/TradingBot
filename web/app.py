...
_SKIP_DBS = {"candles.db", "news.db", "scanner.db", "supervisor.db"}

def load_all_bots() -> list[dict]:
    paths = sorted(
        p for p in glob(os.path.join(DB_DIR, "*.db"))
        if os.path.basename(p) not in _SKIP_DBS and "_EUR" in os.path.basename(p) and not os.path.basename(p).startswith("_")
    )
...
def api_staking():
    import time as _t
    now = _t.time()
    if _staking_cache["data"] is not None and now - _staking_cache["ts"] < 300:
        return jsonify(_staking_cache["data"])
    try:
        from bot.config import ExchangeConfig
        from bot.data_feed import build_exchange
        ex = build_exchange(ExchangeConfig())
        raw = ex.private_post_earn_allocations({})
        items = raw.get("result", {}).get("items", [])
        staked = []
        total_eur = 0.0
        for it in items:
            amount_native = float((it.get("amount_allocated") or {}).get("total", {}).get("native", 0) or 0)
            amount_usd    = float((it.get("amount_allocated") or {}).get("total", {}).get("converted", 0) or 0)
            rew_native = float((it.get("total_rewarded") or {}).get("native", 0) or 0)
            rew_usd    = float((it.get("total_rewarded") or {}).get("converted", 0) or 0)
            if amount_native <= 0 and rew_native <= 0:
                continue
            asset = it.get("native_asset", "?")
            try:
                rate = _eur_rate("USD")  # USD -> EUR conversion
            except Exception:
                rate = 1.0 / 1.1
            amount_eur = amount_usd * rate
            rew_eur    = rew_usd * rate
            total_eur += amount_eur
            payout = it.get("payout") or {}
            est_rew = float((payout.get("estimated_reward") or {}).get("native", 0) or 0)
            staked.append({"asset": asset, "amount_native": amount_native, "amount_eur": round(amount_eur, 4), "rewarded_native": rew_native, "rewarded_eur": round(rew_eur, 4), "est_weekly_native": est_rew})
        staked.sort(key=lambda x: -x["amount_eur"])
        result = {"staked": staked, "total_eur": round(total_eur, 4)}
        _staking_cache["data"] = result
        _staking_cache["ts"]   = now
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e), "staked": [], "total_eur": 0.0})
