...
def parse_args():
    p = argparse.ArgumentParser(description="Supervisor Bot – Regime-Erkennung")
...
def _analyze(exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int) -> tuple | None:
    ...
    Gibt (regime, adx_val, atr_pct) oder None bei Fehler zurück.
    """
    try:
...
def _write(db_path: str, regime: str, adx_val: float, atr_pct: float, dry_run: bool):
    """Schreibt Supervisor-Ergebnisse in die Bot-DB."""
    ...
    if dry_run:
        log.info(
            ...
            f"rsi_buy_max={tmpl['rsi_buy_max']} | atr_sl_mult={tmpl['atr_sl_mult']}"
        )
        return
...
def run_once(exchange: ccxt.Exchange, db_dir: str, timeframe: str, limit: int, dry_run: bool):
    ...
    for db_path in db_paths:
        ...
        _write(db_path, regime, adx_val, atr_pct, dry_run)
